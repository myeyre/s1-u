...
public class ConvToByte {
	public static String charToString (byte[] bytes,int start,int end) {
		Charset cs = Charset.forName ("UTF-8");
		ByteBuffer bb = ByteBuffer.allocate (end-start);
		byte[] bs = new byte[end-start];   
	    System.arraycopy(bytes, start, bs, 0, end-start); 
		bb.put (bs);
		bb.flip ();
	...
	public static String toString2(byte[] bytes,int start,int end,boolean a){
		StringBuilder buffer = new StringBuilder();
		System.out.println("######Length######A:"+start+":"+end);
		
		byte[] bs = new byte[end-start];  
		System.out.println("######Length######B:"+bs.length);
		System.arraycopy(bytes, start, bs, 0, end-start); 
		System.out.println("######Length######C:"+bs.length);
		
		for(int i=start;i<bs.length;i++){
			buffer.append(String.format("%s", bs[i]));
		}
